...
class DiagramCacheMeta with _$DiagramCacheMeta {
  const factory DiagramCacheMeta({
    ...
    @Default(168) int clientCacheTtlHours, // 7 days default
...
// ─────────────────────────────────────────────────────────────────────
// 7. DIAGRAM CACHE SERVICE (client-side)
// ─────────────────────────────────────────────────────────────────────

/// Client-side diagram cache manager.
/// Pre-fetches diagrams for the student's current learning frontier,
/// serves from local cache during sessions (no network latency).
abstract class DiagramCacheService {
  /// Pre-fetch diagrams for concepts the student is likely to encounter next.
  /// Called after each session ends and on app startup.
  /// Budget: max 50MB of diagram cache per subject.
  Future<void> prefetchForFrontier({
    required List<String> frontierConceptIds,
    required String subject,
  });

  /// Get a cached diagram. Returns null if not cached (triggers background fetch).
  Future<ConceptDiagram?> getCachedDiagram({
    required String conceptId,
    DiagramType? preferredType,
  });

  /// Get all challenge cards for a concept (for game-mode sessions).
  Future<List<ChallengeCard>> getChallengeCards({
    required String conceptId,
  });

  /// Evict stale diagrams (older than TTL or for concepts the student has mastered).
  Future<int> evictStale();

  /// Total cache size in bytes (for storage management UI).
  Future<int> getCacheSizeBytes();

  /// Invalidate all diagrams for a concept (triggered by CurriculumPublished event).
  Future<void> invalidateConcept(String conceptId);
}

// ─────────────────────────────────────────────────────────────────────
// 8. SUBJECT DESIGN TOKENS (consistent visual identity per STEM domain)
// ─────────────────────────────────────────────────────────────────────
