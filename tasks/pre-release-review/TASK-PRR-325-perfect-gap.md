# TASK-PRR-325: Event-sourcing rebuild verification test (replay events → state byte-for-byte)
...
Event-sourcing rebuild verification test (replay events → state byte-for-byte).
