...
public partial class Program
{
    ...
    builder.Services.AddSingleton<Cena.Actors.Cas.ICasRouterService, Cena.Actors.Cas.CasRouterService>(); builder.Services.AddCenaCasPersistence(); // PRR-252
    ...
    // TASK-E2E-A-04-BE: parent-bind invite issuer/consumer service. Persists
    // ParentBindInviteDocument under jti and mints HS256 JWTs. The
    // ParentChildBindingMarten registration was already wired earlier in
    // Program.cs (line ~353) so the consume endpoint can call GrantAsync.
    builder.Services.AddScoped<Cena.Actors.Parent.IParentBindInviteService,
        Cena.Actors.Parent.ParentBindInviteService>();
    ...
    app.MapAuthEndpoints(); app.MapCatalogEndpoints(); app.MapSubscriptionEndpoints(); app.MapSubscriptionManagementEndpoints(); app.MapBankTransferEndpoints(); app.MapStripeWebhook(); app.MapParentDashboardEndpoints(); app.MapHouseholdDashboardEndpoints(); app.MapTutorHandoffEndpoints(); app.MapTypedStepsDiagnosticEndpoint(); app.MapSessionAttemptModeEndpoints(); app.MapMeEndpoints(); app.MapApplicableDiscountEndpoint(); app.MapEntitlementEndpoints(); app.MapTestProbeEndpoint(); app.MapParentBindEndpoints(); // prr-011; prr-220 ADR-0050; EPIC-PRR-I / ADR-0057 pricing + /api/me/subscription + /bank-transfer PRR-304 + Stripe webhook + parent dashboard + PRR-324 household dashboard + PRR-325 tutor-handoff HTML report; PRR-384 typed-steps accessibility path; STB-00 Me/Profile; PRR-436 e2e DB-boundary probe; TASK-E2E-A-04-BE parent-bind
